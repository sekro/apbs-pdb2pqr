...
void SurfaceExtract(TeTraMesh* volmesh, SurfaceMesh* surfmesh)
{
  ...
  surfmesh_face = (INT3VECT*)malloc(sizeof(INT3VECT)*volmesh->nf);
 
  /* Classify the vertices into boundary or interior */
  boundary_flag = (char *)malloc(sizeof(char)*volmesh->nv);
  for (n = 0; n < volmesh->nv; n++) 
    boundary_flag[n] = 0;
  
  // Walk over all tetrahedrons of the Volume mesh
  face_num = 0;
  num_vertices = 0;
  for (n = 0; n < volmesh->nf; n++) {
    printf("%2.1f%% done \r", 100.0*(double)(n)/(double)(volmesh->nf-1));
    fflush(stdout);
  ...
  vertex_index = 0;
  vertex_indices = (int*)malloc(sizeof(int)*volmesh->nv);
  for (n = 0; n < volmesh->nv; n++) {
    if (boundary_flag[n]) {
...
SurfaceMesh * SurfaceMesh_merge(SurfaceMesh *surfmesh_in0, SurfaceMesh *surfmesh_in1)
{
  ...
  surfmesh = SurfaceMesh_ctor(surfmesh_in0->nv + surfmesh_in1->nv, 
			      surfmesh_in0->nf + surfmesh_in1->nf);


  for (i = 0; i < surfmesh_in0->nv; i++) {
  ...
  for (i = 0; i < surfmesh_in1->nv; i++) {
    surfmesh->vertex[i+surfmesh_in0->nv].x = surfmesh_in1->vertex[i].x;
    surfmesh->vertex[i+surfmesh_in0->nv].y = surfmesh_in1->vertex[i].y;
    surfmesh->vertex[i+surfmesh_in0->nv].z = surfmesh_in1->vertex[i].z;
  }
  
  for (i = 0; i < surfmesh_in0->nf; i++) {
    surfmesh->face[i].a = surfmesh_in0->face[i].a;
    surfmesh->face[i].b = surfmesh_in0->face[i].b;
    surfmesh->face[i].c = surfmesh_in0->face[i].c;
  }
  for (i = 0; i < surfmesh_in1->nf; i++) {
    surfmesh->face[i+surfmesh_in0->nf].a = surfmesh_in1->face[i].a+surfmesh_in0->nv;
    surfmesh->face[i+surfmesh_in0->nf].b = surfmesh_in1->face[i].b+surfmesh_in0->nv;
    surfmesh->face[i+surfmesh_in0->nf].c = surfmesh_in1->face[i].c+surfmesh_in0->nv;
  }
