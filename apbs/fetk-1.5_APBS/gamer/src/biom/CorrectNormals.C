...
void SurfaceMesh_correctNormals(SurfaceMesh* surfmesh)
{
  ...
  stack = (int*)malloc(sizeof(int)*surfmesh->nf);
  visited = (unsigned char*)malloc(sizeof(unsigned char)*surfmesh->nf);
  ...
  while (1) {
    ...
    while (start < end) {
      if (((start) % 500) == 0) {
	printf("%2.2f%% done (%08d)\r", 100.0*(float)(start+progress)/(float)(surfmesh->nf), start+progress);
	fflush(stdout);
      }
