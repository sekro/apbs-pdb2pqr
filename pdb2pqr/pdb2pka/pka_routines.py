...
class pKaRoutines:
    ...
    def __init__(self, protein, routines, forcefield, apbs_setup, output_dir, maps=None, sd=None,
                 restart=False, pairene=1.0, curve_meth='gc', test_mode=False):
        """
    ...
    def runpKa(self, ghost=None):
        ...
        if ghost:
            ...
            if self.curve_meth == 'gc':
                self.calculatepKaValue(self.curve_meth)
            else:
                self.calculatepKaValueMC()
    ...
    def calculatepKaValueMC(self):
        """
        Calculate the pKa value using Monte Carlo
        """
        #
        ...
        #
        self.routines.write( "acidbase: %s\n" % (acidbase))
...
#        import pdb; pdb.set_trace()
        ...
        # Format the C++ output so that it's the same structure as graph cut's
        # output so that they may both use the same subsequent code.
        count=0
#        pka_values = {}
#        for pKa in self.pKas:
        ...
        for pKa in self.pKas:
            ...
            for titration in pKaGroup.DefTitrations:
                ...
                charges[name]={}
                bits = name.split('_')
                tpl = (bits[3].split(':')[1], bits[1], bits[2])
                curves[tpl] = []
                foo = []
                for x in range(int(num_pHs)):
                    ...
                    curves[tpl].append((pH, pKavals[count]))
                    foo.append(pKavals[count])
                    pH=pH+pH_step
                    

#                print 'max', max(foo), 'min', min(foo)
                print("that thing", name, len(charges[name]))
        ...
        for name, curve in curves.iteritems():
            min_ = min([x[1] for x in curve])
            if min_ < 0:
                curves[name] = [(x[0], x[1]-min_) for x in curve]

                
        _pka, _ph = self.find_pka_and_pH(curves)
        print "PKA:", _pka
        print "PH:", _ph
        ...
        X.write_titration_curve(self.output_files['titcurv_dir'],
                                charges,
                                format='pdb2pka')
        ####### - 4
        #
    ...
    def find_pH_at_0_5(self, name, charges, is_base):
        ...
        self.ph_at_0_5[tuple(name.split('_')[:3])] = ph_at_0_5
    ...
    def calculatepKaValue(self, method):
        """
        Calculate the pKa Value

        We use a graph cut method because it gives the right answer.
        We can also use Monte Carlo, which I'm sure also gives the "right answer".
        """
        #
        # First we need to correct all the interaction energies to make sure that terms aren't
        # counted twice
        ...
        if method == 'gc':
            self.routines.write("Using Graph Cut\n")
            ...
            #
            # Call Graph Cut to do it's thing.
            curves = get_titration_curves(protein_complex)

        elif method == 'mc':
            self.routines.write("Using Monte Carlo\n")
            #
	    # Matrix needs to be linear for transport to c++
	    #
	    linear=[]
	    ...
	    pKavals=FAST.calc_pKas(phstart,phend,phstep)
            ...
            # The C++ module generates pka and curve data.  For consistency, we'll use the same pka
            # and ph calculations as graph cut.  To make this work, we need to extract the curve
            # data from the C++ module output, and reformat them so that both acid and base curves
            # are between 0 and 1.  For some reason the C++ code generates curves for acids between
            # 0 and -1.
            count = 0
            # skip over the pKa information
            for pKa in self.pKas:
            ...
            for pKa in self.pKas:
                ...
                for titration in pKaGroup.DefTitrations:
                    ...
                    for x in range(int(num_pHs)):
                        count=count+1
                        # Fix the pH value formatting
                        pH=float(int(pKavals[count]*100))/100.0
                        count=count+1
                        charges[name][pH]=pKavals[count]
                        curves[tpl].append((pH, pKavals[count]))
                        pH=pH+pH_step
            ...
            for name, curve in curves.iteritems():
                min_ = min([x[1] for x in curve])
                if min_ < 0:
                    curves[name] = [(x[0], x[1]+1) for x in curve]

        else:
            raise ValueError('Expected gc or mc', method)
            

        # Save the titration curves
    ...
    def find_pka_and_pH(self, curves):
        ...
        for name, curve in curves.iteritems():
            ...
            try:
                ...
                warning = "WARNING: UNABLE TO CACLCULATE PKA FOR {name}, Attempted to find log10(0).\n".format(name=name)
                print warning,
                self.warnings.append(warning)
